...
for (var i = 0; i<finalBlocks.length;i++){
    filledBlocks = new Array(finalBlocks[i].length)
    for (var j = 0; j<finalBlocks[i].length; j++){
...
var fill = function(x,y){
    ...
    if ((finalBlocks[x+1][y] == true || filledBlocks[x+1][y]) && (finalBlocks[x-1][y] == true || filledBlocks[x-1][y]) && (finalBlocks[x][y+1] == true || filledBlocks[x][y+1]) && (finalBlocks[x][y=1] == true || filledBlocks[x][y-1])) {
        return;
    }
    if (!finalBlocks[x+1][y]) {
        fill(x+1,y);
    }
    if (!finalBlocks[x-1][y]) {
        fill(x-1,y);
    }
    if (!finalBlocks[x][y+1]) {
        fill(x,y+1);
    }
    if (!finalBlocks[x][y-1]) {
        fill(x,y-1);
    ...
    for (var i = 0; i<finalBlocks.length; i++){
        for (var j = 0; j<finalBlocks[i].length; j++){
            if (!finalBlocks[i][j] && !filledBlocks[i][j]){
                if (filledBlocks[i+2][j] !== true && filledBlocks[i-2][j] !== true && filledBlocks[i+2][j] !== true && filledBlocks[i+2][j] !== true) {
                    continue;
                }
                else {
                    ...
                    else if (finalBlocks[i+1][j] && filledBlocks[i+2][j]){
                        finalBlocks[i+1][j] = false;
                    }
                    else if (finalBlocks[i][j+1] && filledBlocks[i][j+2]){
